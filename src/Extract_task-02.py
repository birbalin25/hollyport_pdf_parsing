...
dbutils.widgets.text("max_tokens", "4000")
dbutils.widgets.text("prompt", "")
dbutils.widgets.text("util_path", "")

catalog = dbutils.widgets.get("catalog")
schema = dbutils.widgets.get("schema")
MODEL_ENDPOINT = dbutils.widgets.get("model")
MAX_TOKENS = int(dbutils.widgets.get("max_tokens"))
prompt_param = dbutils.widgets.get("prompt")
util_path = dbutils.widgets.get("util_path")

# Put the bundle's util/ dir on sys.path so `import pdf_table_utils` resolves.
if util_path and util_path not in sys.path:
...
# ---------------------------------------------------------------------------
# Standalone fallback prompt. The deployed job overrides this with the 'prompt'
# job parameter (variables.yml is the source of truth); see the override below.
# ---------------------------------------------------------------------------
...
tables_df = (spark.read.table(t("raw_parsed"))
...
tables_df.write.mode("overwrite").option("overwriteSchema", "true").saveAsTable(t("parsed_table_elements_staging"))
staging_count = spark.table(t("parsed_table_elements_staging")).count()
...
# MAGIC -- select * from serverless_stable_14ey07_catalog.hollyport.parsed_table_elements_staging
...
tables_df = spark.table(t("parsed_table_elements_staging"))
...
# variables.yml (the 'prompt' job parameter) is the source of truth; fall back to
# the inline template above only when running standalone with no prompt provided.
if prompt_param.strip():
    PROMPT_TEMPLATE = prompt_param

# Escape template for SQL string literal (double any single quotes for the SQL concat)
...
tables_with_llm = tables_df.withColumn(
    ...
    """)
    # Alternatives: databricks-claude-sonnet-4-6  databricks-gpt-oss-20b  databricks-qwen3-next-80b-a3b-instruct
...
result_df.write.mode("overwrite").option("overwriteSchema", "true").saveAsTable(t("all_table_data_batch"))
...
# MAGIC %sql
# MAGIC -- Interactive check (disabled for job runs; table lives in the parameterized catalog/schema):
# MAGIC -- select * from serverless_stable_14ey07_catalog.hollyport.all_table_data_batch
...
source_df = spark.table(t("all_table_data_batch"))    
...
t("extracted_table_data_variant")
)
print(f"Saved {spark.table(t('extracted_table_data_variant')).count()} rows")
# display(spark.table(t("extracted_table_data_variant")))
