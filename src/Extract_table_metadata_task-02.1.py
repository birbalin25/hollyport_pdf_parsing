...
page_elements_df = (spark.read.table(t("raw_parsed"))
            .select(
                F.col("file_name"),
...
result_df.write.mode("overwrite").option("overwriteSchema", "true").saveAsTable(t("table_metadata"))
# display(result_df)
...
df = spark.read.table(t("table_metadata"))
...
grouped_df.write.mode("overwrite").option("overwriteSchema", "true").saveAsTable(t("grouped_table_metadata"))
# display(grouped_df)
